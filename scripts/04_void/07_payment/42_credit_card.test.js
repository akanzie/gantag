 ...
 * * テスト観点：
 * * クレジットカードで支払った売上取引は誤打訂正ができる。（誤打訂正可）
 * 
 * ---
 * ### テスト方法/シナリオ
 * | Step | 手順 | エンドポイント |
 ...
 * | 7 | 【誤打訂正】支払登録 (クレジットカード) | `/void/addpayment` |
 ...
 * * * \+ void_payments にクレジットカード支払が含まれること
 ...
 * * #### 8.【誤打訂正】取引終了 `/void/end`
 * * \- レシートデータに「クレジット」「誤打訂正」が含まれ、返金金額が売上取引と同額であることを確認
 */
export function TC_040742001_VoidCreditPayment() {
  group("TC_040742001 クレジットカード払い", () => {
    const step = {
      ...
      voidPayment: CommonFunction.getFullDesc(ENDPOINT.VOID_PAYMENT, "【誤打訂正】支払登録 (クレジットカード)"),
    ...
    const payment = voidCartInfo?.payments?.find(p => p.paid_cd === PAID_METHOD.CREDIT.PAID_ITEMS.CREDIT.PAID_CODE);
    ...
    TestHelper.voidEnd(step.voidEnd, {
      ...
      CHECK.createStatusCodeCheck(),
      CHECK.createEqualsCheck({
        name: "Verify receipt data must contain information: クレジット, 誤打訂正 and refund amount is equal sales transaction",
        expected: true,
        actual: (res) => {
