 ...
 * ### 期待結果
 * * #### 3. 小計 `/sales/subtotal`
 * * \- sales.cartinfoデータを取得する（TC_069で実施する）。
 * * #### 4. 支払登録 `/sales/addpayment`
 ...
 */
export function TC_020742001_CreditCardPayment() {
  group("TC_020742001 クレジットカード払い", () => {
    const step = {
    ...
    const paidGroupCode = PAID_METHOD.CREDIT.GROUP_CODE;
    const paidCode = PAID_METHOD.CREDIT.PAID_ITEMS.CREDIT.PAID_CODE;
    const details = ENVIRONMENT.CREDIT_DETAIL;
    ...
    const creditPayment = salesPayments?.find(p => p.paid_cd === paidCode);
    ...
    const payment = refundCartInfo?.payments?.find(q => q.paid_cd === PAID_METHOD.CREDIT.PAID_ITEMS.CREDIT.PAID_CODE);
    ...
    TestHelper.refundPayment(step.refundPayment, {
      ...
      CHECK.createStatusCodeCheck(),
      CHECK.createEqualsCheck({
        name: "Verify void payments contains クレジット payment",
        expected: {
        ...
        actual: (res) => {
          const creditVoidPayment = res.result?.cartinfo?.void_payments?.find(p => p.paid_cd === paidCode);
          return {
            paidCd: creditVoidPayment?.paid_cd,
            paidName: creditVoidPayment?.paid_name,
            paidAmount: creditVoidPayment?.paid_amount,
          };
        },
      }),
      CHECK.createEqualsCheck({
    ...
    TestHelper.refundEnd(step.refundEnd, {
      ...
      CHECK.createStatusCodeCheck(),
      CHECK.createEqualsCheck({
        name: "Verify receipt data must contain information: クレジット, ご返金 and refund amount is equal sales transaction",
        expected: true,
        actual: (res) => {
          const stringTotalPaidAmount = CommonFunction.convertToCurrency(totalPaidAmount);
          return CommonFunction.includesItems([
            PAID_METHOD.CREDIT.PAID_ITEMS.CREDIT.PAID_NAME,
            "ご返金",
