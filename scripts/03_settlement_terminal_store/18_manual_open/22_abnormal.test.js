 ...
 * * \- business_day を取得する
 * * #### 2. 端末精算 `/settlement`
 * * \- business_day が step 1 で取得した business_day と一致していることを確認する
 * * #### 3.端末を除外する store`/settlement/exclude-pos` (pos_cd: 04)
 * * \- POS端末は除外するPOSか確認
 * * * \+ pos_cd: 04
 ...
 * * * \+ エラーコード: "STL0006"
 */
export function TC_031822001_ManualOpenAbnormal() {
  group("TC_031822001 端末精算～店舗精算～手動開局（異常系）", () => {
    const preStep = {
    ...
    const excludePosCd = ENVIRONMENT.POS_CD_04; // Test data. It is not subject to store settlement.
    const cashInfoList = {
    ...
    const businessDay = CommonFunction.getSegmentBefore("T", businessDayCashInspection);
    ...
    TestHelper.executePosSettlement(step.settlement, {}, [
      ...
      CHECK.createEqualsCheck({
        ...
        actual: (res) => res.result?.before_business_day,
      }),
    ]);
