 ...
 * * * \+ count: 30
 * * 2. storeSettlement
 * * * \+ settlement_datetime: step2での「business_day」
 * * 3. storeSettlementExcludePos: 
 * * * \+ pos_cd: "04"
 * * * \+ settlement_business_day: step2での「business_day」
 * * 4. POS精算情報
 * * * \+ user_cd:  pos_user_01
 ...
 * * \- business_day を取得
 * * #### 3. 在高点検 `/cash/inspection`
 * * \- business_day が Step2 で取得した business_day と一致していることを確認
 * * #### 4. 端末精算 `/settlement`
 * * \- business_day が当日の日付となっていることを確認
 * * #### 7. 在高点検 `/cash/inspection`
 * * \- business_day が Step2 で取得した business_day と一致していることを確認
 * * #### 8. 端末精算 `/settlement`
 * * \- business_day が当日の日付となっていることを確認
 ...
 * * \- business_day が Step2 で取得した business_day と一致していることを確認
 */
export function TC_031621001_AutoOpenNormal() {
  group("TC_031621001 端末精算～店舗精算～自動開局（正常系）", () => {
    const preStep = {
    ...
    const currentDateFormat = CommonFunction.getSegmentBefore(" ", currentDate);
    const cashInfoList = {
